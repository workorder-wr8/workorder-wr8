...
from (select to_char(w.datecreated,'Mon DD Day') as "day", count(w.datecreated) as "workorders"  from workorders w join properties p on p.id = w.propertyid where p.id = $1 group by "day" order by "day" asc) a
